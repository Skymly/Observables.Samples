...
Runnable sample applications for **[Skymly/Observables](https://github.com/Skymly/Observables)** using the **`0.2.2`** NuGet meta-packages on [nuget.org](https://www.nuget.org/packages/Observables.Events.R3/0.2.2) (Events, RestAPI, SignalR, Mqtt, WebSocket, Grpc, Sse, Nats, Postgres, and Redis).
...
`Directory.Build.props` sets `ObservablesPackageVersion` to **`0.2.2`** and `UseLocalObservables=false` by default.
