...
Console sample for **`Observables.SignalR.R3`** `0.1.0-preview6` (declarative hub interface â†?R3 `Observable` + `HubService.For`).
...
- Source-generated proxy registration via module initializer (`RegistrationDemo` â€?no network in CI)
...
2. `HubConnectionBuilder.WithUrl("https://localhost:â€?hub").Build()` then `await connection.StartAsync()`.
