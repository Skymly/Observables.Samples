...
using Observables.Samples.Grpc.Reactive.Api;

namespace Observables.Samples.Grpc.Reactive.Demos;
...
internal static class RegistrationDemo
{
    public static void Run()
    {
        ...
        try
        {
            _ = GrpcService.For<ISmokeHubReactive>(null!);
            Console.WriteLine("  unexpected: null invoker accepted");
        }
        catch (ArgumentNullException)
        {
            Console.WriteLine(
                "  GrpcService.For<ISmokeHubReactive>: factory registered (ArgumentNullException on null invoker)");
        }
