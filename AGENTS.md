...
- **Default**: NuGet **`ObservablesPackageVersion`** (`0.1.6-preview1`) via `Observables.Events.R3` / `Observables.RestAPI.R3` / `Observables.SignalR.R3` / `Observables.Mqtt.R3` / `Observables.WebSocket.R3` / `Observables.Grpc.R3` / `Observables.Sse.R3` / `Observables.Nats.R3` on nuget.org.
