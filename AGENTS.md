...
- **Default**: NuGet **`ObservablesPackageVersion`** (`0.1.0`) via `Observables.Events.R3` / `Observables.RestAPI.R3` / `Observables.SignalR.R3` / `Observables.Grpc.R3` / `Observables.Sse.R3` on nuget.org.
